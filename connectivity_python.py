...
for i in range(len(entity_labels)):
    connectivity[entity_labels[i]] = {
        "neighbors": [], "wall": [], "area": 0.0, "thickness": 0.25, "volume": 0.0}
...
for i in range(len(temp)):
    ...
    x1min, y1min, x1max, y1max = temp[i][0].item(
    ), temp[i][1].item(), temp[i][2].item(), temp[i][3].item()
    ...
    for j in range(i, len(temp)):
        if i != j:
            x2min, y2min, x2max, y2max = temp[j][0].item(
            ), temp[j][1].item(), temp[j][2].item(), temp[j][3].item()
